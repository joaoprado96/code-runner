...
def create_random_data():
    trans_ids = [f"B{random.randint(10, 1500)}" for _ in range(1500)]
    prog_ids = [f"X{random.randint(0, 9)}{chr(random.randint(65, 90))}" for _ in range(150)]
    terms = [str(random.randint(10, 99)) for _ in range(20)]
    grupos = [f"G{random.randint(10, 99)}" for _ in range(20)]
    
    serv_negocios = ["PAGAMENTO DE BOLETOS", "TRANSFERÊNCIAS", "SAQUES", "INVESTIMENTOS"]
    grupo_suporte = ["BOLETO PAGO (S000900)", "TRANSFERÊNCIA (S000901)", "SAQUES (S000902)"]
...
if __name__ == "__main__":
    ...
    # print(json.dumps(random_data[first_trans_id], indent=4))
